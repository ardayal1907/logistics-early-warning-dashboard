...
Late shipments and untracked emissions both carry hidden costs — customer churn, penalty clauses, and regulatory exposure. This project turns raw shipment data into two things a logistics manager can act on **today**:

1. **A predictive risk score** for every shipment (`Delay_Risk_Probability`, 0–1) and an easy-to-read `Risk_Level` (High / Medium / Low), generated by a Random Forest classifier trained on vendor, route, weather, and traffic features.
2. **A carbon cost estimate** for the fleet, translating CO₂ emissions directly into a dollar figure (`Carbon Tax Impact`) using a configurable carbon price assumption.

> ⚠️ **Scope note:** this is a portfolio/demonstration project built on **synthetic
> data**. It shows how such a pipeline is *constructed and validated*; it is not
> evidence of real-world predictive performance. See
> [What This Project Demonstrates](#-what-this-project-demonstrates) and
> [Limitations](#️-known-limitations) before quoting any figure from it.

The result is a 2-page Power BI dashboard that lets operations and sustainability stakeholders answer, at a glance:
> *"Which shipments are about to fail, why, and what is our environmental exposure costing us?"*

**Target audience:** Logistics operations managers, supply chain analysts, ESG/sustainability reporting teams, and data leadership evaluating AI-augmented BI solutions.
...
The Power BI semantic model implements the following core measures:
...
> [Key point 8](#-what-this-project-demonstrates).
...
An interactive single-shipment scorer sits alongside the batch pipeline. Enter a
vendor rating, route conditions and load; get a **calibrated** delay probability, a
risk tier, and the calculated carbon cost.
...
The app requires `models/production_risk_model.pkl`, which ships with the repository
and is regenerated by step 3 of the pipeline. If it is missing, run
`python src/ml_delay_risk_pipeline.py` first.
...
pip install -r requirements.txt
```
Power BI Desktop (free) is required to open and interact with the `.pbix` report.
...
# → data/raw/smart_logistics_data.csv
```

### 2. Run the Star Schema ETL
```bash
python src/etl_star_schema.py
# → data/processed/: Dim_Vendor.csv, Dim_Route.csv, Dim_Date.csv, Fact_Shipments.csv
```

### 3. Train the ML model and score delay risk
```bash
python src/ml_delay_risk_pipeline.py
# → data/processed/Fact_Shipments_with_ML.csv (adds Delay_Risk_Probability, Risk_Level)
# → models/production_risk_model.pkl + metadata JSON (consumed by the Streamlit demo)
```
...
1. Open `powerbi/Lojistik_Erken_Uyari_Paneli.pbix` in Power BI Desktop.
2. If prompted, point the data source connections to the CSVs in `data/processed/`.
3. Load `Dim_Date.csv` and create its relationship — see [Power BI setup](#-power-bi-setup-after-adding-dim_date).
4. Click **Refresh** to load the latest scored data.
5. Explore Page 1 (Executive Summary) and Page 2 (Early Warning Panel).
...
8. **The `.pbix` file is not refreshed automatically.** Data source paths must be
   ...
   [Power BI setup](#-power-bi-setup-after-adding-dim_date).
...
This project uses fully synthetic data generated for demonstration and portfolio purposes. Feel free to fork and adapt.
